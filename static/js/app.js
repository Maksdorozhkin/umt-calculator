...
function fillProductData(machineId, product) {
    document.getElementById(`cavitations${machineId}`).value = product.cavitations;
    document.getElementById(`cycles${machineId}`).value = product.cycles_per_minute;
    document.getElementById(`piecesPerBox${machineId}`).value = product.pieces_per_box;
    document.getElementById(`boxesPerPallet${machineId}`).value = product.boxes_per_pallet;
}
...
function setupProductForms() {
    for (let i = 1; i <= 7; i++) {
        ...
        if (calcBtn) calcBtn.addEventListener('click', () => calculateProduction(i));
        ...
        // Add downtime button
        const addBtn = document.querySelector(`.add-downtime-btn[data-machine="${i}"]`);
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            const productData = {
                ...
                cycles_per_minute: parseFloat(document.getElementById('newCycles').value),
                pieces_per_box: parseInt(document.getElementById('newPiecesPerBox').value),
                boxes_per_pallet: parseInt(document.getElementById('newBoxesPerPallet').value)
            };
...
async function loadProductList() {
    ...
    list.innerHTML = productCache.map(p => `
...
async function saveProduct(machineId) {
    ...
    const productData = {
        ...
        cavitations: parseInt(document.getElementById(`cavitations${machineId}`).value),
        cycles_per_minute: parseFloat(document.getElementById(`cycles${machineId}`).value),
        pieces_per_box: parseInt(document.getElementById(`piecesPerBox${machineId}`).value),
        boxes_per_pallet: parseInt(document.getElementById(`boxesPerPallet${machineId}`).value)
    };
...
async function logDowntime(machineId) {
    const downtimeType = document.getElementById(`downtimeType${machineId}`).value;
    let duration = 0;

    if (downtimeType === 'custom') {
        duration = parseInt(document.getElementById(`downtimeDuration${machineId}`).value);
...
async function loadDowntimeLog(machineId) {
    try {
        ...
        if (downtimes.length === 0) {
            logContainer.innerHTML = '<div class="empty-state" style="padding: 20px;"><p style="font-size: 0.85rem; color: var(--text-muted);">Нет записей о простоях</p></div>';
            updateAvailableTime(machineId);
            return;
        }
        ...
        logContainer.innerHTML = downtimes.map(d => {
...
async function calculateTape() {
    ...
    const requiredPieces = parseInt(document.getElementById('requiredPieces').value);
    ...
    try {
        ...
        const result = await response.json();
        document.getElementById('tapeNeeded').textContent = result.tape_needed.toLocaleString('ru-RU');
        document.getElementById('tapePlus10').textContent = result.tape_plus_10.toLocaleString('ru-RU');
...
window.deleteDowntime = async function (machineId, downtimeId) {
    ...
    try {
        const response = await fetch(`/api/downtime/${downtimeId}`, { method: 'DELETE' });
        const result = await response.json();
