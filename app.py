...
def calculate_production(product, machine_id):
    ...
    remaining_boxes = math.floor(total_boxes) - (full_pallets * product.boxes_per_pallet)
...
def index():
    ...
    return render_template('index.html', machines=machines, shift_type=shift_type, all_products=all_products)

@app.route('/api/products', methods=['GET'])
def get_all_products():
    products = Product.query.order_by(Product.name).all()
    return jsonify([{
        'id': p.id, 'name': p.name, 'cavitations': p.cavitations,
        'cycles_per_minute': p.cycles_per_minute,
        'pieces_per_box': p.pieces_per_box, 'boxes_per_pallet': p.boxes_per_pallet
    } for p in products])
...
def save_product():
    try:
        ...
        if product:
            ...
            product = Product(
                name=name, cavitations=data['cavitations'],
...
def get_product(id):
    ...
    return jsonify({
        'id': product.id, 'name': product.name, 'cavitations': product.cavitations,
        'cycles_per_minute': product.cycles_per_minute,
        'pieces_per_box': product.pieces_per_box, 'boxes_per_pallet': product.boxes_per_pallet
    })
...
def assign_product_to_machine(machine_id):
    try:
        ...
        if machine_product:
            ...
            machine_product = MachineProduct(machine_id=machine_id, product_id=product_id)
...
def get_machine_downtime(machine_id):
    try:
        ...
        downtimes = DowntimeLog.query.filter(
            ...
            DowntimeLog.timestamp >= shift_start,
            DowntimeLog.timestamp <= shift_end
        ).order_by(DowntimeLog.timestamp.desc()).all()
        
        return jsonify([{
            'id': d.id, 'downtime_type': d.downtime_type,
            'duration_minutes': d.duration_minutes,
            'timestamp': d.timestamp.isoformat(), 'note': d.note
        } for d in downtimes])
    except Exception as e:
...
def calculate_machine_production(machine_id):
    try:
        ...
        return jsonify({
            'pallets': pallets, 'boxes': boxes, 'pieces': pieces,
